	...
	$.fn.sSelect = function(options) {
		...
		return this.each(function(){
			...
			var opts = $.extend(defaults, options),
				...
				$containerDiv = $('<div class="newListSelected '+div_class+'" tabindex="0" id="'+div_id+'"></div>'),
				$newUl = $('<ul class="newList '+div_class+'"></ul>'),
				itemIndex = -1,
				currentIndex = -1,
				keys = [],
				prevKey = false,
			...
			function newUlPos(){
				...
				if (containerPosY+newUlHeight >= docHeight){
					...
					// .selectTxt margin afeta o posicionamento
					$newUl.css({top: (containerHeight+14)+'px', height: newUlHeight});
					$input.onTop = false;
				}
			}
